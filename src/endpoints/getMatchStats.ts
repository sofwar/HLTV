...
export const getMatchStats = (config: HLTVConfig) => async ({
    ...
    const getPlayerTopStat = ($: CheerioStatic, index: number): PlayerStat => {
        const playerEls = $($('.most-x-box').get(index))

        return {
            id: Number(playerEls.find('.name > a').attr('href')!.split('/')[3]),
            name: playerEls.find('.name > a').text(),
            value: Number(playerEls.find('.valueName').text())
        }
    }
    ...
    const teamEls = m$('.match-info-box .team-logo')

    const team1: TeamStat = {
        id: Number(popSlashSource(teamEls.first())),
    ...
    const team2: TeamStat = {
        id: Number(popSlashSource(teamEls.last())),
    ...
    const playerOverviewStats: PlayerStats[] = toArray(m$('.stats-table tbody tr')).map(rowEl => {
        ...
        return {
            ...
            kills: Number(
                rowEl.find('.st-kills').contents().first().text()
            ),
            hsKills: Number(
                rowEl.find('.st-kills .gtSmartphone-only').text().replace(/\(|\)/g, '')
            ),
            assists: Number(
                rowEl.find('.st-assists').contents().first().text()
            ),
            flashAssists: Number(
            ...
            KAST: Number(
                rowEl.find('.st-kdratio').text().replace('%', '')
            ),
